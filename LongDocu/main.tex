...
\subsection{Cardiovascular mathematics (Chapter 6)}
...
\subsubsection*{BLOOD MORPHOLOGY AND VISCOMETRIC PROPERTIES}
\textit{Blood is a multi-component mixture with complex rheological character-
istics. It consists of multiple cellular elements:
\begin{enumerate}
\item \textbf{red blood cells} - RBCs (erythrocytes), the most numerous of the formed elements (about 98\%) are tiny biconcave discoid particles, filled with a fluid, which are involved in oxygen and carbon dioxide transport
\item \textbf{white blood cells} - WBCs (leukocytes) are much less numerous than RBCs, they have nuclei, and are classified into two groups - are involved in the organisms defence against invasion by bacteria and viruses - Rather little is known of the mech- anical properties of the WBCs. It has been argued that they are stiffer than RBCs, because in a collision between a red and a white cell in flowing blood, it is the former which mainly deforms
\item \textbf{platelets (thrombocytes)}, small discoid cell fragments containing various chemicals such as serotonin, thrombin, ADP, are much smaller than erytrocytes - they have a negligible effect on the mechanics of normal blood, compared to erythrocytes.
\item \textbf{The cellular elements are suspended in an aqueous polymer solution, the plasma}, containing electrolytes as well as organic molecules such as metabolites, hormones, enzymes, antibodies and other proteins and representing approximately 55\% of the blood volume. Plasma transports nutrients as well as wastes throughout the body
\end{enumerate}
}


{\color{blue} \textit{\textbf{We focus particular attention on the red blood cells because they are the
only cells which significantly influence the mechanical properties of blood. They do this because they are present in very high concentration (..) comprising about 40 to 45\% of its volume (hematocrit). The rheology of blood is therefore primarily determined by the behaviour of the erythrocytes at different shear rates.}}}



\subsubsection*{Blood viscosity and viscoelasticity}
...
blood.
\subsubsection*{time-independent constitutive equations }
...
\midrule
\textbf{Power Law} & $  \eta = k \Dot{\gamma}^{n-1}$ &  For $n<1$ shear thinning behaviour + possible to obtain
numerous analytical solutions, - zero shear rate behaviours are
unphysical, - the
asymptotic limit as shear rate goes to infinity is zero \textbf{It does not describe the low
and high shear rate regions} & 
\cite{AdeliaSequeira2017} \\
\textbf{Power-law model from Walburn and Schneck} & $  \eta = k \Dot{\gamma}^{n-1}$\hfill  , $k=C_1exp(C_2H_t)$ \hfill, $n=1-C_3H_t$ & Dependence of the viscosity
on the haematocrit and total fibrinogen and globulins proteins, TPMA, \textbf{was developed for low shear rates, however, and was not valid
for certain shear rate regimes seen in their pulsatile flow simulations} & 
\cite{Robertson2009_cardiovascular} \\
\textbf{General form }  &$  \mu(\Dot{\gamma})=\mu_{\infty}+(\mu_0 - \mu{\infty})F(\Dot{\gamma})$ \hfill or $     \frac{\mu(\Dot{\gamma})- \mu_{\infty} }{\mu_0 - \mu{\infty}}= F(
\Dot{\gamma})$ & Viscosity functions with bounded and non-zero limiting values of viscosity, $(\mu_0$ and $\mu{\infty})$ are the asymptotic viscosity values at zero and infinite shear rates and $F(\Dot{\gamma})$ is a shear dependent function, satisfying the natural limit condition, i.e. goes to 1 for strain rate to zero and goes to 0 if strain rate goes to infinity  & \cite{Bessonov2016_MethodsBloodFlow} \\
\textbf{Carreau}  &$F(\Dot{\gamma})= (1+(\lambda\Dot{\gamma})^2)^{\frac{n-1}{2}}$ & were seen to agree well in their predictions and were preferred over the power-law model which has an unbounded viscosity at zero shear-rate &\cite{Shibeshi2005_Carreau}\\ 
\textbf{Carreau-Yasuda}  & $(1+(\lambda\Dot{\gamma})^a)^{\frac{n-1}{a}}$ & velocity shows a flattened profile (larger region of higher velocity), reaching a lower
maximum value & \cite{Robertson2009_cardiovascular}\cite{Morozov2015_nonnewtonian} !!!\\
...  & ... & ... & ...\\
\bottomrule
\end{tabular}
...
\item In a An Overview of Some Mathematical Models of Blood Rheology the importance of Platelet activation and blood coagulation is discussed - \textit{However they are by far the most sensitive of all the components of blood to chemical and physical agents, and play also a significant role in blood rheology ...} \newline
{\color{red}I will neglect biochemical processes?}
